        ...
        <div class="registerBox">
            <transition name="fade" mode="out-in">
                <div key=1 class="registerInputBox">
                    ...
                    <input id="email" class="inputField" type="email" v-model.trim="email" placeholder="Email"/>
                    <input id="password" class="inputField" type="password" v-model.trim="password" placeholder="Password"/>
                    <input id="repeatPassword" class="inputField" type="password" v-model.trim="repeatPassword" placeholder="Repeat password"/>
                    <a class="redirects" @click="updateRoute('Login')">Login</a>
                    <a class="redirects" @click="updateRoute('ResetPassword')">Reset password</a>
                    <button id="register-btn" class="submitButton" @click="register">Register</button>
                </div>
        ...
        methods: {
            ...
            register: function() {
                if (this.validateForm()) {
                  // TODO: CHANGED REGISTER TO NON CAPITAL LETTERS
                    let data = {
                        email: this.email,
                    ...
                    this.$store.dispatch('register', data)
                        ...
                        .catch(err => {
                                if (err.response.status === 409) {
                                    ...
                                    this.$toaster.error('Password is invalid, passwords require a captial, number and a special character.')
                                } else {
