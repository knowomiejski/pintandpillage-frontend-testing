<template>
    <div data-test="buildingListItemTestId">
        <div :data-test="'buildingList' + building.name" class="buildingListItemContainer" :class="{ disabledOverlay: !canBeBuild()}">

            <img class="buildingIcon" v-bind:src="require('../../assets/buildingIcons/' + building.name + '.png')"/>

            <div class="buildingInformationContainer">
                <h1 data-test="buildingListItemBuildingNameId">{{building.name}}</h1>
                <p id="buildingDescription">{{building.description}}</p>

                <div style="display: flex; flex-direction: row;">
                    <population-frame :checkAvailability='checkAvailability' :population-left="building.populationRequiredNextLevel"></population-frame>
                    <time-frame class="timeComp" :required-time="building.constructionTime"></time-frame>
                </div>

                <div class="resourcesContainer">
                    <resource-item class="resourceItemComp" :checkAvailability='checkAvailability' :resources="building.resourcesRequiredLevelUp"></resource-item>
                </div>

                <div v-for="(value, key) in building.buildingLevelRequiredToLevelup" :key="value.key">
                    <h2 class="requirements" v-if="!hasBuildingRequirement(key, value)" >Requires {{key}} level {{value}}</h2>
                </div>

            </div>

            <button data-test="buildingButtonTestId" :id="'buildingButtonTest'+building.name" :disabled="!canBeBuild()" @click="createBuilding(building.name)">Build</button>
    ...
    export default {
        ...
        methods:{
            createBuilding: function (inputvalue) {
                ...
                this.$store.dispatch("createBuilding", { "villageId": villageId, "buildingType": inputvalue, "position": {"x": this.position.x, "y": this.position.y} })
                    .then(() => this.close())
            },
            close: function () {
                this.$emit('close');
            ...
            canBeBuild: function() {
                ...
                for (const [buildingType, level] of Object.entries(this.building.buildingLevelRequiredToLevelup)) {
                    amountOfLevelrequirementsMet +=  this.hasBuildingRequirement(buildingType, level);
                }
                if(amountOfLevelrequirementsMet !== Object.keys(this.building.buildingLevelRequiredToLevelup).length){
                    return false;
                }
                return true;
            },
            hasBuildingRequirement(buildingName, level) {
                for (let i = 0; i < this.village.buildings.length; i++) {
                    let building = this.village.buildings[i];
...
<style lang="scss">
    .disabledOverlay{
        #buildingDescription, img, h1, button{
            filter: grayscale(1);
            color: #7f7f7f;
